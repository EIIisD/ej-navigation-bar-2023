...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, iconElement, iconElementAfter, ...props }, ref) => {
    return (
      <div className="relative flex w-full items-center text-sm">
        ...
        {iconElement && (
          <Icon
            name={iconElement}
            className="pointer-events-none absolute left-4 w-4 text-orange"
          />
        )}
        {iconElementAfter && (
          <Icon
            name={iconElementAfter}
            className="absolute right-4 w-4 cursor-pointer text-orange"
          />
        )}
      </div>
    )
